...
import { generateSpec, getYamlSpecs } from "./index"
...
test("getYamlSpecs", async () => {
  const specs = await getYamlSpecs({
    sourceDir: "openapi",
  })
...
test("generates simple spec", async () => {
  ...
  expect(generateSpec({ entryPointSpec: spec, partialSpec })).toMatchSnapshot()
})

test("generates complex spec", async () => {
...
test("finds missing definitions simple", () => {
  const spec = {
    a: {
      b: {
        c: {
...
test("finds missing definitions complex", async () => {
  const spec = await importYamlContent({
    fileName: "openapi/connectors/bundled.yaml",
  })
